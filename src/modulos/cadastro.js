class Cadastro {
  ...
  static validarDadosCadastro(data) {
    ...
    if (
      !data.avatarUrl.includes(".jpeg") &&
      !data.avatarUrl.includes(".jpg") &&
      !data.avatarUrl.includes(".png")
    ) {
    ...
    if (
      ...
      validarLetraMaiuscula === false ||
      validarNumero === false
    ) {
      err["password"] =
        "A senha precisa ter pelo menos 6 dígitos, 1 letra maiuscula e 1 número para ser válida";
    }
  ...
  static direcionarUser(data) {
    ...
    if (
      data.message == "An user with the same username is already registered"
    ) {
      const span = document.createElement("span");
